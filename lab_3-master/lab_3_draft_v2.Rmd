...
If we find that the severity of punishment is associated with crime rate (with greater severity associated with lower crime), we may consider stricter policies around judicial practices so that people receive more severe punishments if sentenced with a crime.
...
One can see that there is one potential outlier that also has leverage, meaning that it is an extreme outlier (observation #6). 
...
For reasons discussed at the EDA section of this project, `wfed` and `taxpc` will be included as economic context variables as they are a proxy of local wealth and overall economic development.
