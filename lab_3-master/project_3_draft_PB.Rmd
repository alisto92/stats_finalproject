...
We will be modeling our dependent variable (crime rate), using the following variables as candidate policy variables. We focus on variables that represent different economic and criminal justice policies, which can be influenced via various policy decisions taken by our candidate. 
...
PB - comment : I think MW policy is a good ideia in theory, but with the avaiable data, we have no idea how a shift in MW will impact sector weekly wage. The politician cannot make a promisse about rising wages in manufacturing, for example, in which employees make more than 1 MW. Besides, the economic literature is not clear about the impact  of MW on emmployment levels - I mean, the politician can increase MW, but this can lead to higher unemployment rates, which at the end can boost crime. And we have no data on employment level in the dataset to control for it.   
...
If we find that the probability of arrest is associated with crime rate (with higher likely of getting caught associted with lower crime rates), we may consider more efficient policies around police practice that encourage more arrests.

In this case, we would be interested in the variable named `prbarr`, defined as the 'probability' of arrest.

*Probability of conviction* 

If we find that the probability of conviction is associated with crime rate (with higher conviction rates associted with lower crime rates), we may consider stricter policies around court practices so that more arrests lead to convictions.

In this case, we would be interested in the variable named `prbconv`, defined as the 'probability' of conviction.

*Probability of sentencing* 

If we find that the probability of sentencing is associated with crime rate (with higher sentencing rates associted with lower crime rates), we may consider stricter policies around judicial practices so that more convictions lead to sentencing.

In this case, we would be interested in the variable named `prbpris`, defined as the 'probability' of prison sentence.

*Severity of punishment* 

If we find that the severity of punishment is associated with crime rate (with greater severity associted with lower crime rates), we may consider stricter policies around judicial practices so that people receive more severe punishments if sentenced with a crime.

In this case, we would be interested in the variable named `avgsen`, defined as the average sentence in days.
...
PB - comment: For the reasons discussed above, I believe that wage variables should be included in the Contextual Variables. 
...
The # of police police officers per capita (measured by the variable `polpc`) is a very intuitive example of crime deterrant, as it in theory increases the likelyhood of being caught in the act of the crime and is also a measure of the capacity of the State to enforce the law. 

As our job is to advise a policy maker, it's very important that we feel safe about the causal interpretation of our findings. After all, the ideia is to propose policies with a significant impact on people's lives. A cross section of counties tough does not allow us to link police presence to crime rates with a causal interpretation. This is because one can expect the counties with larger crime rates in a year t would consider necessary to have a larger number of police officers in on the streets in year t, which can imply a positive correlation between presence of police and crime rates with no causal meaning. 

Thus, in this exercise we opted not to include police per capita as a candidate of policy variable - instead, we we'll include it as a control variable to access the effectiveness of shifts in other criminal justice variables (probabilities of arrest, conviction and prision sentence) on crime rates. 
...
That being so, we believe the best strategy is to evaluate the arrest/conviction/sentencing variables to see whether they are related to different levels of crime - whether they could be having any effect as crime deterrents would be important for policymakers to know. 
