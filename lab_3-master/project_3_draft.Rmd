...
We also include a range of contextual variables that help to control for local characteristics (for example, demographic and geographic indicators) that could influence crime and the efficacy of these policies but are not actionable by a local government. We aim to produce a model robust to these, that can help guide decisions at the state level and help reduce crime across these counties. 
...
If we find that the efficiency of arrest is associated with crime rate (with higher punishment associted with lower crime), we may consider stricter policies around police practice that encourage more arrests. 
...
If we find that the efficiency of sentencing is associated with crime rate (with higher sentencing rates associted with lower crime), we may consider stricter policies around judicial practices so that more convictions lead to sentencing.
...
As our job is to advise a policy maker, it’s very important that we feel safe about the causal interpretation of our findings. After all, the idea is to propose policies with a significant impact on people’s
lives. For statistical rigor, we also need to make sure that causal effects go one way (that we have reason to believe that our independent variables have causal effects on our dependent variables, and not the reverse). A cross section of counties does not allow us to link police presence to crime rates with a causal interpretation. This is
...
Our target variable (`crmrte`) appears to be of sufficient quality for our analyses and thus we will use it as our dependent variable. 
...
We can see that `wrtd` (Wholesale/Retail/Trade) wages are closest to the minimum wage - which would suggest that wages in this sector would be most influenced by changes to the minimum wage. The next sector that would be influenced is `wser` (Service Industry). However, `wser` appears to have an outlier (in one county, the  sector's average weekly wage is over $2000, which is well above the weekly wages for any other sector as well as `wser` in other counties). Additionally, `wser` may be a messier measure of income because many in that industry work in restaurants and make additional income through tips - however, some people may not report this income. Meanwhile, those working in Wholesale/Retail/Trade may be more likely to work in places like malls, which would not provide additional income as tips and would be a better measure of low-income wages. 
...
Our target (dependent) variable is the log of crime rate [log(`crmrte`)]. Taking the log makes intuitive sense, as the estimated coefficients will represent the percentage increase in crime.  


In the first model spacification, we used the following variables to model crime: (i) log of eff_cj, our composed indicator of the criminal justice system efficiency, which is defined as the of the (`prbarr` * `prbconv` * `prbpris`), (ii) log of average sentence [log(`avg_sen`)'] and (iii) log of weekly wages in wholesale and retail sectors [log(`wtrd`)].    

The first two variables are crime deterrants that can be actionable by the local government. Wages in wholesale and retail sectors were picked as a proxy of minimum wage, Indeed, this sector's wage is close to the minimum and would be more likely to affect by minimum wage policy. Rising local minimum wage can act as a crime deterrant policy as well, as it can reduce income inaquality within the county, which arguably reduces incentives for crime.    

However, our economic policy variable,`wtrd`, was significantly related to crime rate but not in the expected direction - the higher the wages, the higher the crime. Since `wtrd` and `density` are positively correlated, we thought that perhaps wages in this sector would be higher in large cities and this omitted variable could be influencing the direction of the relationship between `wtrd` and `crmrte`.  

Thus in the second model specification, we added `density` as a control variable, to check if, controling for density,  the unexpected positive coefficient estimate on `wtrd` would persist. As the coefficient estimate on `wtrd` remained positive even controlling density, as the reson for that is not clear to us, we opted to exclude `wtrd` from in the modeling strategy.  

Our third model specification features only indicators of the criminal justice system, namely log of eff_cj, our composed indicator of the criminal justice system efficiency, and the log of average sentence. Approximately 30% of the variation in crime rate can be explained by these variables. The estimated coefficient on log of average sentence has the extected sign (negative), but is not statistically significant at the 10% level. Equation 3 thus suggests that increasing the ratio of criminal justice practices to crime (increasing arrests, convictions, and sentencing) could help decrease the crime rate. 

In our fourth model specification, we add a range of contextual variables that help to control for local characteristics that could influence crime and the efficacy of these crime policies but are not actionable by a local government. More specifically, in equation 4 we control for the mix of offenses, the percentage of young male in population, the percentage of minority, tax revenue per and geographic variables). We concluded that our model robust to these contextual variables, reinforcing our reading that increasing the ratio of criminal justice practices to crime helps decrease the crime rate. 

[Discuss more about transformations, estimated values and implications]

```{r}
data2 <- data2 %>% mutate(eff_cj = prbarr*prbconv*prbpris)

m1 <- lm(log(crmrte) ~ log(eff_cj) + log(avgsen) + log(wtrd), data = data2) # with law enforcement vars and one wage var 
m2 <- lm(log(crmrte) ~ log(eff_cj) + log(avgsen) + log(wtrd) + log(density), data = data2) # adding density
m3 <- lm(log(crmrte) ~ log(eff_cj) + log(avgsen), data = data2) # back to a simpler spacification (only CJS vars)
m4 <- lm(log(crmrte) ~ log(eff_cj) + log(avgsen) + log(density) + mix  + pctymle + pctmin80 + west + central + urban  + log(taxpc) , data = data2) #also controlling for offence mix, demographic and geographic data


stargazer(m1, m2, m3, m4, type="text", title = "Results Table",
