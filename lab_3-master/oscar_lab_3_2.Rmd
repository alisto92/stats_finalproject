...
```{r}
hist(data2$crmrte * data2$density)
```


```{r}
hist(data2$density * data2$pctmin80)
```

```{r}
hist(data2$density * data2$polpc)
```

```{r}
crime_density_md = lm(log(crmrte * density) ~ log(eff_cj), data = data2)
crime_density_md$coefficients
```

```{r}
plot(crime_density_md)
```

```{r}
crime_density_2_md = lm(log(crmrte) ~ log(eff_cj) + log(pctmin80 * density), data = data2)
crime_density_2_md$coefficients
```

```{r}
plot(crime_density_2_md)
```
